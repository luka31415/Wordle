...
def Filler(greenLetters, yellowLetters, possibleWords):
    if len(greenLetters) + len(yellowLetters) > 2 and len(possibleWords) > 2:
        return True
...
def ProcessAnswer(word, clue, yellowLetters, yellowPositions, blackLetters, greenLetters, greenPositions, doubleYellowLetters, guessedWords):
    ...
    if Filler(greenLetters, newYellowLetters, possibleGuesses):
