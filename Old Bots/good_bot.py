...
def FillerChoiceAdvanced(word, greenPositions, yellowLetters, yellowPositions):
    for letter in word:
        if letter in yellowLetters and yellowPositions[yellowLetters.index(letter)] == word.index(letter):
            return False
        elif letter in yellowLetters and word.index(letter) in greenPositions:
            return False
    return True

def FillerChoice(greenLetters, greenPositions, yellowLetters, yellowPositions, possibleWords, wordList):
    letters = []
    words = []
    ...
    for word in wordList:
        if FillerChoiceAdvanced(word, greenPositions, yellowLetters, yellowPositions):
            wordListNew.append(word)
    
    for word in wordListNew:
        words.append(LetterCounter(word, letters))

    word = wordListNew[words.index(max(words))]
    return word
...
def ProcessAnswer(word, clue, yellowLetters, yellowPositions, blackLetters, greenLetters, greenPositions, doubleYellowLetters, guessedWords):
    ...
    if len(possibleGuesses) > 2:
        choice = FillerChoice(greenLetters, greenPositions, yellowLetters, yellowPositions, possibleGuesses, allWords)
        madeChoice = True
...
def Play(guess, guessedWords, yellowLetters, yellowPositions, blackLetters, greenLetters, greenPositions, doubleYellowLetters):
...
def Main():
    while True:
        ...
        while numOfGuesses < 6:
            guess = Play(guess, guessedWords, yellowLetters, yellowPositions, blackLetters, greenLetters, greenPositions, doubleYellowLetters)
